...
%%%%%%%%%%%%% BASICS %%%%%%%%%%%%%%%%%
                       ...
                       datayearstart=double(data.scan_line_year(1));
                       datadoystart=double(data.scan_line_day_of_year(1));
                       datautcstart=double(data.scan_line_UTC_time(1));%in milliseconds
                       datayearend=double(data.scan_line_year(end));
                       datadoyend=double(data.scan_line_day_of_year(end));
                       datautcend=double(data.scan_line_UTC_time(end));%in milliseconds
...
moonflagMoonCloserButNotSignificant=0*ones(length(scnlinyr),1);
...
% execute script "mooncheck" for setting moon flags (building on information in the
% logfiles generated by mhs_cl-program)
mooncheck  % comment this line and the "In case of Moon intrusions lines",
            % if you do not want to apply moonchecks, e.g. because you don't have the
            % right logfile with the intrusions for your dataset.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                              
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%    END calibration counts %%%%%%%%%%%                      
     ...
     %%%%%%%%%%%%%%%%%%%%      PRE-PROCESSING of DSV and IWCT counts
     
                                
     
     
     
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% In case of Moon Intrusion:

                               if ~isempty(scnlinallviewsbad)
                               
                               % All Views bad: set DSV counts to mean from
                               % closest good scanline
                               usablelines=setdiff(scanlinenumbers,scnlinallviewsbad); %get usablelines as difference of all lines and bad lines
                               for i=1:length(scnlinallviewsbad) 
                               [val,closestgoodline(i)]=min(abs(double(usablelines)-double(scnlinallviewsbad(i))));%find closest good line for every badline
                               end
                               dsv_mean(scnlinallviewsbad,1)=dsv_mean(closestgoodline,1);
                               dsv_mean(scnlinallviewsbad,2)=dsv_mean(closestgoodline,2);
                               dsv_mean(scnlinallviewsbad,3)=dsv_mean(closestgoodline,3);
                               dsv_mean(scnlinallviewsbad,4)=dsv_mean(closestgoodline,4);
                               dsv_mean(scnlinallviewsbad,5)=dsv_mean(closestgoodline,5);
                                 
                               
%                                    % All Views bad: set DSV counts to mean from last scan
%                                % before Moon Intrusion
%                                dsv_mean(scnlinallviewsbad,1)=dsv_mean(scannumbers(1)-1,1);
%                                dsv_mean(scnlinallviewsbad,2)=dsv_mean(scannumbers(1)-1,2);
%                                dsv_mean(scnlinallviewsbad,3)=dsv_mean(scannumbers(1)-1,3);
%                                dsv_mean(scnlinallviewsbad,4)=dsv_mean(scannumbers(1)-1,4);
%                                dsv_mean(scnlinallviewsbad,5)=dsv_mean(scannumbers(1)-1,5);
                               end
                               
                               % if only some views are contaminated, we
                               % use the good ones. Assignment to
                               % dsv_mean is done after qualitychecks_DSV
%                                if ~isempty(scnlinoneviewok)
%                                % At least one view ok: only use views that
%                                % are ok for calculating mean. "meandsvch1okviews" has ben calculated already in mooncheck.                            
%                                dsv_mean(scnlinoneviewok,1)=meandsvch1okviews;
%                                dsv_mean(scnlinoneviewok,2)=meandsvch2okviews;
%                                dsv_mean(scnlinoneviewok,3)=meandsvch3okviews;
%                                dsv_mean(scnlinoneviewok,4)=meandsvch4okviews;
%                                dsv_mean(scnlinoneviewok,5)=meandsvch5okviews;
%                                end
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
                            ...
                            % that use the median-tests on IWCT and DSV counts
                                calculate_AllanDeviation_onCounts_withoutQualflags
     
     
                            %%%%%% Quality checks on DSV  and IWCT and flag setting  
     
                            % execute script for quality checks and appropriate calculation of mean on DSV
                            qualitychecksDSV
                            
                            % for moon intrusion in some views: 
                            % set mean here, after quality checks, since
                            % moon intrusion might not be captured by
                            % quality checks.
                            if ~isempty(scnlinoneviewok)
                               % At least one view ok: only use views that
                               % are ok for calculating mean. "meandsvch1okviews" has ben calculated already in mooncheck.                            
                            ...
                            % execute script for quality checks and
                            % appropriate calculation of mean on IWCT
                            qualitychecksIWCT
                           ...
                           % USABLE CHANNELS
                            ...
                            %%%% IWCT AND DSV: SMOOTHING: scanline-averaging (3 before, 3 after
                               % scanline X) 
                               qualflag_incompl_av_datagap_startfile=zeros(length(scanlinenumbers),1);
                               %calculating the average over 7 scanlines for the centre scanline
                               for scanline=1:length(data.scan_line_year);
                                   if scanline < 4   %FIX make variable that conatins all scanlines of orbit
                                       ns=scanline-1;
                                       qualflag_incompl_av_datagap_startfile(scanline)=1;
                                    elseif scanline > length(data.scan_line_year)-3
                                        ns=length(data.scan_line_year)-scanline;
                                        qualflag_incompl_av_datagap_startfile(scanline)=1;
                                    elseif isnan(scnlinyr(scanline+3))||isnan(scnlinyr(scanline-3))
                                       ns=2;
                                        %if there is data gap due to
                                        %missing scanline! set ns to 2 or 1
                                        %depending on distance to NaN
                                        %value. AND SET QUALITY FLAGS
                                        qualflag_incompl_av_datagap_startfile(scanline)=1;
                                        
                                    elseif isnan(scnlinyr(scanline+2))||isnan(scnlinyr(scanline-2))
                                       ns=1;
                                       qualflag_incompl_av_datagap_startfile(scanline)=1;
                                    elseif isnan(scnlinyr(scanline+1)) || isnan(scnlinyr(scanline)) || isnan(scnlinyr(scanline-1))
                                       ns=0;
                                       qualflag_incompl_av_datagap_startfile(scanline)=1;
                                    else
                                        ns=3;
                                    ...
                                    for selectchannel=1:5
                                    countIWCT_av(selectchannel,scanline)= scanlineavCounts(iwct_mean(scanline-ns:1:scanline+ns,selectchannel),scanline,ns);
                                    countDSV_av(selectchannel,scanline)= scanlineavCounts(dsv_mean(scanline-ns:1:scanline+ns,selectchannel),scanline,ns);
                                    end
                               end    
                                
                               
                            % UNCERTAINTY of DSV and IWCT counts
                                
                                % calculate the noise on counts with Allan
                                % Deviation
                                calculate_AllanDeviation_onCounts
                               % The result from the Allan Deviation
                               % calculation (1 per window of good scan lines) is 
                               % assigned to all good scan lines in this
                               % window. Bad scan lines get a NAN as noise
                               % value.
                               % The uncertainty in the 7-scanline-average of counts is
                               % computed with linear addition (not
                               % quadratic), since  we still have 1/f noise that
                               % may deviate the counts all into one direction over a certain period.
                               % This is a "worst case approximation" of uncertainty.
                               % The linear addition leads to
                               % u_C=sum(weights)*dsvcountallandev_scnlin=1*dsvcountallandev_scnlin
                               %FIXME: try out quadratic addition! What
                               %does the result look like int FCDR?
                               %now (03.08.2014) I use the factor 0.4146 to
                               %account for the 7-scanline averaging and
                               %the used weights. 0.4146=sqrt(2*w3^2+2*w2^2+2*w1^2+2*w0^2)
                               u_C_S=0.4146*bsxfun(@times,ones(5,length(data.scan_line_year)),dsvcountallandev_scnlin.'); 
                               
                             
                               u_C_IWCT=0.4146*bsxfun(@times,ones(5,length(data.scan_line_year)),iwctcountallandev_scnlin.');%30*ones(size(countIWCT_av)); %dummy value of 30 so far!
                               
                               % for estimation of earth count noise (do not
                               % use roll av. since earth counts are NOT
                               % averaged!) via scaling
                               u_C_S_0=bsxfun(@times,ones(5,length(data.scan_line_year)),dsvcountallandev_scnlin.');
                               u_C_IWCT_0=bsxfun(@times,ones(5,length(data.scan_line_year)),iwctcountallandev_scnlin.');
    ...
    %%%%%%%%%%%%%%%%%%%        
                         % EARTH COUNTS       
                                ...
                                % execute quality checks on earthview
                                % counts
                                qualitychecksEarthview %watch out! the threshold might need adjustment for MHS!
                               ...
                               %frac=u_C_IWCT(:,1)./countIWCT_av(:,1);
                               %u_C_E=bsxfun(@times,earthcounts,frac);%28*ones(size(earthcounts)); %dummy value of 28 so far!
   %%%%%%%%%%%%%%%%%%%%
                           
                         % PRT TEMPERATURE
                         
                           % coefficients for counts to temp.-conversion
                           % FIXME: PRT counts not yet read out of l1b file, since PRT-Temp provided directly for MHS. FIX 
                           ...
                           % PRT7coeff=[hdrinfo.caltargettempcoeff(25) hdrinfo.caltargettempcoeff(26) hdrinfo.caltargettempcoeff(27) hdrinfo.caltargettempcoeff(28)];
                            ...
                            %%%%%% AVERAGING, QUALITY CHECKS and flag
                            %%%%%% setting for PRT
                            
                            % execute script for quality checks and
                            % appropriate calculation of mean of PRT
                            % sensors
                            qualitychecksPRT
                           ...
                           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
                            ...
                            % MAKE SCANLINE AVERAGING
                            for scanline=1:length(data.scan_line_year);
                                   if scanline < 4   %FIX make variable that contains all scanlines of orbit
                                       ns=scanline-1;
                                    elseif scanline > length(data.scan_line_year)-3
                                        ns=length(data.scan_line_year)-scanline;
                                   elseif isnan(scnlinyr(scanline+3))
                                       ns=2;
                                        %if there is data gap due to
                                        %missing scanline! set ns to 2 or 1
                                        %depending on distance to NaN
                                        %value. AND SET QUALITY FLAGS
                                    elseif isnan(scnlinyr(scanline+2))
                                       ns=1;
                                    elseif isnan(scnlinyr(scanline+1)) || isnan(scnlinyr(scanline))
                                       ns=0;
                                    else
                                        ns=3;
                                   end
                                   
                                    for selectchannel=1:5 % artificially generate IWCTtemp for all channels (the values are the same, we only need this array dimension for later)
                                    IWCTtemp_av(selectchannel,scanline)= scanlineavCounts(ICTtempmean(scanline-ns:1:scanline+ns),scanline,ns); %apply triangualr weighting as for counts
                                    end
                               end    
                            ...
                            % FIXME: random uncertainy on PRT measurements:
                            % Noise on PRT
                            %use Allan Dev to calc. noise on the 5
                            %PRTs and avergage over 5 PRTs (as for 4 views
                            %of DSV,IWCT). Since we only take the
                            %"onlygoodPRTscnlin", i.e. only scan lines
                            %where all 5 PRTs are good, we always avergage
                            %over 5 PRT sensors in the AllanDev.
                            % Artificially generate matrix with entries for
                            % all 5 channels. Just handy for processing for
                            % now.
                            calculate_AllanDeviation_onPRT
                            
                            u_T_IWCT_noise=0.4146*PRTallandev_scnlin.';
...
%                          thermcoeff4=-8.02652E-08;%double(extract_int32(header, 605, 608))/10^15;
      ...
      %%%%%%%%%%%%%                
                      ...
                      [~,profile]=fill_missing_scanlines(double(data.scan_line_UTC_time),scanlinenumbers_original,double(data.status_word_profile)); % info on profile from header! CHECK!
...
%                         alpha=[0.0002 0.0015 -0.0022 -0.0022 0.0021];
      ...
      %%%%%%%%%%%%%%%%
                           ...
                           %convert to degrees
                           Antenna_position_iwctview_4views=hdrinfo.antenna_position_conversion_factor*(theta_IWCT_counts)-180;%-180 to get nadir at zero
...
%                                 gE=gEold-3*(1-gEold);
                               ...
                               Antenna_corrcoeff_earthview=g_prime;
                               %Antenna_corrcoeff_earthview=gE;
                               
                               % UNCERTAINTY
                               u_Antenna_corrcoeff_earthview=0.5*(1-Antenna_corrcoeff_earthview); %estimate uncertainty of 50% of the correction to an efficiency of 1
