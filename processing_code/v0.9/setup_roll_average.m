...
% count value for the ok-views to the dsv_mean.
    ...
    if ~isempty(scnlinoneviewok)
       ...
       dsvmean_per_line(scnlinoneviewok,1)=meandsvch1okviews;
       dsvmean_per_line(scnlinoneviewok,2)=meandsvch2okviews;
       dsvmean_per_line(scnlinoneviewok,3)=meandsvch3okviews;
       dsvmean_per_line(scnlinoneviewok,4)=meandsvch4okviews;
       dsvmean_per_line(scnlinoneviewok,5)=meandsvch5okviews;
    end
  
    
% write NAN incase of "all views bad" in moon intrusion    
    if ~isempty(moonflagAllViewsBad)
    dsvmean_per_line(scnlinallviewsbad,1)=nan;
    dsvmean_per_line(scnlinallviewsbad,2)=nan;
    dsvmean_per_line(scnlinallviewsbad,3)=nan;
    dsvmean_per_line(scnlinallviewsbad,4)=nan;
    dsvmean_per_line(scnlinallviewsbad,5)=nan;
    end
