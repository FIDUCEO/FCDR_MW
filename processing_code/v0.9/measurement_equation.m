...
R_Eprime=nan*ones(5,90,scanlinenumbers(end));
Tb_Eprime=nan*ones(5,90,scanlinenumbers(end));
R_E=nan*ones(5,90,scanlinenumbers(end));
Tb=nan*ones(5,90,scanlinenumbers(end));
...
R_CMB0(channel,:)=planck(invcm2hz(wavenumber_central(channel)),bandcorr_a(channel)+bandcorr_b(channel)*2.72548);
R_CMB(channel,:)=planck(invcm2hz(wavenumber_central(channel)),bandcorr_a(channel)+bandcorr_b(channel)*2.72548+dT_c(channel));
%assume R_sat (i.e. R_pl)
R_Sat(channel,:)=planck(invcm2hz(wavenumber_central(channel)),bandcorr_a(channel)+bandcorr_b(channel)*1.75*290);%*2.72548 *instrtemp
end
...
qualbit_tb_badrange(btemps<90 | btemps>310)=1;
