...
AllanDev_countIWCT_av(repmat(moonflagAllViewsBad,[1 5]).'==1)=nan;
...
qualflag_DSV_fewerlines_used(logical(num_usedlinesDSV<7)==1)=1;
qualflag_IWCT_fewerlines_used=zeros(5,totalnumberofscanlines);
qualflag_IWCT_fewerlines_used(logical(num_usedlinesIWCT<7)==1)=1;
...
%     usable_mat{chn}=~(sparse_expanded_UNusablelines_DSV_chns{chn}.*sparse_expanded_UNusablelines_DSV_chns{chn}.');
    ...
    qualflag_PRT_fewerlines_used(logical(num_usedlinesPRT<7)==1)=1;
