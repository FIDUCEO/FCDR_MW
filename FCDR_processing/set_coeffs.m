...
% make sure you set the correct path to the two .mat files (depending on where you keep them, they are provided in the same directory of the repository as this script set_coeffs.m)
...
%% NOAA 15 AMSUB
%%%%%% Count limits %%%%% 1st comment are limits suggested by me
count_thriwct_ch1=[23000 33000];%[23000 33000];[23000 28000];
count_thriwct_ch2=[23000 33000];%[23000 33000];[23000 28000];
count_thriwct_ch3=[11000 33000];%[22000 33000];%[9000 28000];
count_thriwct_ch4=[7000 30000];%[18000 30000];%[7000 23000];
count_thriwct_ch5=[5000 28000];%[15000 28000];%[5000 22000];

count_thrdsv_ch1=[17000 28000];%[17000 28000];[16000 22000];
count_thrdsv_ch2=[17000 28000];%[17000 28000];[17000 23000]
count_thrdsv_ch3=[10000 28000];%[17000 28000];%[8000 24000]; 
count_thrdsv_ch4=[6000 25000];%[15000 25000];%[6000 20000]; 
count_thrdsv_ch5=[4000 23000];%[12500 23000];%[4000 18000]; 
...
u_RFI_counts=zeros(3,5);
%earthviews (uncertainty of RFI correction)
%stdev over Fovs and first corrected year
%=ceil(std(std(abs(delta_C_E(9:20,1:5,:)),0,3),0,1))
u_RFI_counts(1,1)=1;%26;
u_RFI_counts(1,2)=1;%300;
u_RFI_counts(1,3)=1;%40;
u_RFI_counts(1,4)=20;%665;
u_RFI_counts(1,5)=2;%144;
% DSV and IWCT are zero for N15 since all is put into earthviews.

% Extra uncertainty in Kelvin due to contamination of reference month for RFI
% correction 
%= stdev_over_fovs(bias_vsN18(referencemonth,fovs)) code: std(bias(8,:,:),0,3)
u_RFI_offset(1)=0.3;% this channel is not corrected, but we know that there is RFI, hence assing an uncertainty that reflects the overall RFI impact
u_RFI_offset(2)=4;% this channel is not corrected, but we know that there is RFI, hence assing an uncertainty that reflects the overall RFI impact
u_RFI_offset(3)=0.5;% rough estimate of a Kelvin as uncertainty that reflects the imperfection of the RFI correction (namely assigning all bias to RFI...although there might be other effects at work)
u_RFI_offset(4)=2;
u_RFI_offset(5)=2;
...
count_thriwct_ch3=[9000 27500];%[19000 27500];%[7200 27500]; 
count_thriwct_ch4=[4000 24000];%[17000 24000];%[2200 24000];
count_thriwct_ch5=[11000 26000];%[21000 26000];%[8700 26000];

count_thrdsv_ch1=[15000 20000];
count_thrdsv_ch2=[19000 23000];
count_thrdsv_ch3=[9000 23000];%[16000 23000];%[7000 23000];
count_thrdsv_ch4=[4000 21000];%[14000 21000];%[2000 21000];
count_thrdsv_ch5=[11000 23000];%[18000 23000];%[8500 23000];
...
u_RFI_counts(1,2)=1;%300;
u_RFI_counts(1,3)=1;%40;
u_RFI_counts(1,4)=1;%665;
u_RFI_counts(1,5)=1;%144;
% DSV and IWCT are zero for N15 since all is put into earthviews.

% Extra uncertainty in Kelvin due to contamination of reference month for RFI
% correction 
%= stdev_over_fovs(bias_vsN18(referencemonth,fovs)) code: std(bias(8,:,:),0,3)
u_RFI_offset(1)=0;%this channel is not corrected, hence no offset in reference month
u_RFI_offset(2)=0;
u_RFI_offset(3)=0.2;
u_RFI_offset(4)=0.27;
u_RFI_offset(5)=0.49;

% %old RFI uncert. (rough)
% % uncertainty in counts due to RFI
% u_RFI_counts=zeros(3,5);
% %earthviews
% u_RFI_counts(1,1)=11;
...
%stdev over Fovs and first corrected year
%=ceil(std(std(abs(delta_C_E(9:20,1:5,:)),0,3),0,1))
u_RFI_counts(1,1)=2;%26;
u_RFI_counts(1,2)=2;%300;
u_RFI_counts(1,3)=3;%40;
u_RFI_counts(1,4)=3;%665;
u_RFI_counts(1,5)=3;%144;
% DSV and IWCT are zero for N15 since all is put into earthviews.
...
%type in manually the values from fdf.dat file
                           ...
                           % for each channel there are 90 values
                                ...
                                % values in /scratch/uni/u237/sw/AAPP7/AAPP/data/preproc/fdf.dat  for each
                                % instrument per satellite
% old correction (was copied from N15 in AAPP)
%                                 %gE fraction of signal that comes from earth
%                                 gE(1,:)=reshape([
%                                 0.997074 0.997264 0.997422 0.997581 0.997737 0.998033 0.998167 0.998285 0.998391 0.998501 
%                                 0.998597 0.998700 0.998823 0.998946 0.999081 0.999322 0.999399 0.999474 0.999532 0.999595
...
% values from NOAA19 (attempt using these for N18 instead of amsub ones)
%gE fraction of signal that comes from earth
gE(1,:)=reshape([
...
u_RFI_counts=zeros(3,5);
%earthviews (uncertainty of RFI correction)
%estimate one count to account for slight zig zag pattern
u_RFI_counts(1,1)=1;
u_RFI_counts(1,2)=1;
u_RFI_counts(1,3)=1;
u_RFI_counts(1,4)=1;
...
% correction 
%= stdev_over_fovs(bias_vsN18(referencemonth,fovs)) code: std(bias(8,:,:),0,3)
u_RFI_offset(1)=0; %is uncorrected, hence there is no reference month
u_RFI_offset(2)=0;
u_RFI_offset(3)=0.08;
u_RFI_offset(4)=0.11;
u_RFI_offset(5)=0;
...
% uncertainty in counts due to RFI
u_RFI_counts=zeros(3,5);
% MA does not get RFI corrected
%earthviews (uncertainty of RFI correction)
%estimate one count to account for slight zigzag pattern over scan
u_RFI_counts(1,1)=1;
u_RFI_counts(1,2)=1;
u_RFI_counts(1,3)=1;
u_RFI_counts(1,4)=1;
