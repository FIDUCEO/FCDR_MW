...
quotientarr=bsxfun(@times,bsxfun(@times,quotient,ones(1,size(Tb,3))),ones(5,size(Tb,3),number_of_fovs));
quotientarr=permute(quotientarr,[1 3 2]);
TCMBarr=bsxfun(@times,bsxfun(@times,countDSV_av,ones(1,size(Tb,3))),ones(5,size(Tb,3),number_of_fovs));
TCMBarr=permute(TCMBarr,[1 3 2]);
uCSarr=bsxfun(@times,u_C_S_0,ones(5,length(Tb),number_of_fovs));
uCSarr=permute(uCSarr,[1 3 2]);
%actual computation of u_C_E
u_C_E=uCSarr+bsxfun(@times,(earthcounts-TCMBarr),quotientarr);

% uncertainty on Earthcounts due to RFI
u_C_E_RFI=zeros(size(u_C_E));
u_C_E_RFI(1,:,:)=u_RFI_counts(1,1);
u_C_E_RFI(2,:,:)=u_RFI_counts(1,2);
u_C_E_RFI(3,:,:)=u_RFI_counts(1,3);
u_C_E_RFI(4,:,:)=u_RFI_counts(1,4);
u_C_E_RFI(5,:,:)=u_RFI_counts(1,5);

% uncertainty on IWCTcounts due to RFI
u_C_IWCT_RFI=zeros(size(u_C_E));
u_C_IWCT_RFI(1,:)=u_RFI_counts(3,1);
u_C_IWCT_RFI(2,:)=u_RFI_counts(3,2);
u_C_IWCT_RFI(3,:)=u_RFI_counts(3,3);
u_C_IWCT_RFI(4,:)=u_RFI_counts(3,4);
u_C_IWCT_RFI(5,:)=u_RFI_counts(3,5);

% uncertainty on DSVcounts due to RFI
u_C_S_RFI=zeros(size(u_C_E));
u_C_S_RFI(1,:)=u_RFI_counts(2,1);
u_C_S_RFI(2,:)=u_RFI_counts(2,2);
...
bandcorr_a_s_mat=bsxfun(@times,permute(bandcorr_a_s,[1 3 2]),dummy_matrix);
bandcorr_b_s_mat=bsxfun(@times,permute(bandcorr_b_s,[1 3 2]),dummy_matrix);
...
% DSV counts
u_rad_C_S=-1/(Antenna_corrcoeff_earthcontribution_mat).* (R_IWCT_mat-R_CMB_mat).*(earthcounts_mat-countIWCT_av_mat)./(countIWCT_av_mat-countDSV_av_mat).^2 .*(1+nonlincoeff_mat.*(R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat).* (-2*earthcounts_mat+countIWCT_av_mat+countDSV_av_mat)).*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat))));
u_rad_C_S_RFI=u_rad_C_S;

% IWCT Counts
u_rad_C_IWCT=-1/(Antenna_corrcoeff_earthcontribution_mat).* (R_IWCT_mat-R_CMB_mat).*(earthcounts_mat-countDSV_av_mat)./(countIWCT_av_mat-countDSV_av_mat).^2 .*(-1+nonlincoeff_mat.*(R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat).* (-2*earthcounts_mat+countIWCT_av_mat+countDSV_av_mat)).*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat))));
u_rad_C_IWCT_RFI=u_rad_C_IWCT;

% Earth Counts
u_rad_C_E=-1/(Antenna_corrcoeff_earthcontribution_mat).* (R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat) .*(-1+nonlincoeff_mat.*(R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat).* (-2*earthcounts_mat+countIWCT_av_mat+countDSV_av_mat)).*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat))));
u_rad_C_E_RFI=u_rad_C_E;

% Temperature of Blackbody (IWCT temperature)
u_rad_T_IWCT=1/(Antenna_corrcoeff_earthcontribution_mat).*bandcorr_b_mat.*DplanckDT(invcm2hz(wavenumber_central_mat), bandcorr_a_mat+bandcorr_b_mat.*(IWCTtemp_av_mat+dT_w_mat)).*(1+(earthcounts_mat-countIWCT_av_mat)./(countIWCT_av_mat-countDSV_av_mat)+2*nonlincoeff_mat.*(earthcounts_mat-countIWCT_av_mat).*(earthcounts_mat-countDSV_av_mat).*(R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat).^2).*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat))))+(alpha_mat./(2*Antenna_corrcoeff_earthcontribution_mat).*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat)))).*bandcorr_b_mat.*DplanckDT(invcm2hz(wavenumber_central_mat), bandcorr_a_mat+bandcorr_b_mat.*(IWCTtemp_av_mat+dT_w_mat));

% channel frequency
% this calculates derivative wrt frequency. it uses the intermediates steps
% performes outside the fov-loop
u_rad_chnfreq=1/(Antenna_corrcoeff_earthcontribution_mat).*(dRIWCT_df+(earthcounts_mat-countIWCT_av_mat)./(countIWCT_av_mat-countDSV_av_mat).*(dRIWCT_df-dRCMB_df)+2*nonlincoeff_mat.*(earthcounts_mat-countIWCT_av_mat).*(earthcounts_mat-countDSV_av_mat).*(R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat).^2 .*(dRIWCT_df-dRCMB_df).*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat))))-(1-Antenna_corrcoeff_earthcontribution_mat).*dRCMB0_df)+(alpha_mat./(2*Antenna_corrcoeff_earthcontribution_mat).*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat)))).*dRIWCT_df;
...
% performes outside the fov-loop
u_rad_bandcorr_a=1/(Antenna_corrcoeff_earthcontribution_mat).*(dRIWCT_da+(earthcounts_mat-countIWCT_av_mat)./(countIWCT_av_mat-countDSV_av_mat).*(dRIWCT_da-dRCMB_da)+2*nonlincoeff_mat.*(earthcounts_mat-countIWCT_av_mat).*(earthcounts_mat-countDSV_av_mat).*(R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat).^2 .*(dRIWCT_da-dRCMB_da).*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat))))-(1-Antenna_corrcoeff_earthcontribution_mat).*dRCMB0_da)+(alpha_mat./(2*Antenna_corrcoeff_earthcontribution_mat).*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat)))).*dRIWCT_da;

% bandcorrection b
% it uses the intermediates steps
% performes outside the fov-loop
u_rad_bandcorr_b=1/(Antenna_corrcoeff_earthcontribution_mat).*(dRIWCT_db+(earthcounts_mat-countIWCT_av_mat)./(countIWCT_av_mat-countDSV_av_mat).*(dRIWCT_db-dRCMB_db)+2*nonlincoeff_mat.*(earthcounts_mat-countIWCT_av_mat).*(earthcounts_mat-countDSV_av_mat).*(R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat).^2 .*(dRIWCT_db-dRCMB_db).*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat))))-(1-Antenna_corrcoeff_earthcontribution_mat).*dRCMB0_db)+(alpha_mat./(2*Antenna_corrcoeff_earthcontribution_mat).*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat)))).*dRIWCT_db;
...
u_rad_dT_c=-1/(Antenna_corrcoeff_earthcontribution_mat).* (earthcounts_mat-countIWCT_av_mat)./(countIWCT_av_mat-countDSV_av_mat) .*(1+2*nonlincoeff_mat.*(earthcounts_mat-countDSV_av_mat).*(R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat)).*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat)))).*bandcorr_b_s_mat.*DplanckDT(invcm2hz(wavenumber_central_mat), bandcorr_a_s_mat+bandcorr_b_s_mat.*(T_CMB0_mat+dT_c_mat));

% non-linearity coefficient
u_rad_nonlincoeff=1/(Antenna_corrcoeff_earthcontribution_mat).*(earthcounts_mat-countDSV_av_mat).*(earthcounts_mat-countIWCT_av_mat).*(R_IWCT_mat-R_CMB_mat).^2./(countIWCT_av_mat-countDSV_av_mat).^2 .*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat))));

% CMB0 temperature
u_rad_T_CMB0=((-1/(Antenna_corrcoeff_earthcontribution_mat).* (earthcounts_mat-countIWCT_av_mat)./(countIWCT_av_mat-countDSV_av_mat).*bandcorr_b_s_mat.*DplanckDT(invcm2hz(wavenumber_central_mat), bandcorr_a_s_mat+bandcorr_b_s_mat.*(T_CMB0_mat+dT_c_mat)) .*(1+2*nonlincoeff_mat.*(earthcounts_mat-countDSV_av_mat).*(R_IWCT_mat-R_CMB_mat)./(countIWCT_av_mat-countDSV_av_mat)).*(1- alpha_mat./2 .*(cos(deg2rad(2*Antenna_position_earthview_mat))-cos(deg2rad(2*Antenna_position_spaceview_mat)))) -(1-Antenna_corrcoeff_earthcontribution_mat)./Antenna_corrcoeff_earthcontribution_mat).*bandcorr_b_mat.*DplanckDT(invcm2hz(wavenumber_central_mat), bandcorr_a_mat+bandcorr_b_mat.*(T_CMB0_mat)));
...
u_nonrandom_btemps=sqrt(u_btemps_C_S.^2+u_btemps_C_IWCT.^2+u_btemps_Antenna_position_spaceview.^2+u_btemps_T_IWCT_noise.^2); %+u_btemps_Tb_MEprime.^2

%u_common_btemps encompasses all effects that have corelations scales
%larger than one orbit (e.g. the uncertainty is valid for whole orbit)
%u_btemps_chnfreq
%u_btemps_bandcorr_a
%u_btemps_bandcorr_b
%u_btemps_T_IWCT
...
% u_btemps_T_CMB0
% u_btemps_dT_c
% u_btemps_nonlincoeff
% u_btemps_alpha
% u_btemps_Antenna_corrcoeff_earthcontribution
% u_btemps_Antenna_corrcoeff_spacecontribution
% u_btemps_Antenna_corrcoeff_platformcontribution
% u_btemps_radiance_of_platform

% rough estimates have been made for uncertainties of chnfreq,Tcmb0, b and
% a, but they can be neglected.
%+u_btemps_T_CMB0.^2+u_btemps_chnfreq.^2+u_btemps_bandcorr_a.^2+u_btemps_bandcorr_b.^2+  to be
%neglected
